...
$r = 0.21$. We call this a \emph{failure-to-induce}: rather than
picking from a small set of candidate rules as humans do, the model
produces hundreds of different grids, suggesting it never settles on a
rule.
...
tasks to quadrants in Section~\ref{sec:results}, we call the human
...
that nobody can solve. This means the model's spread tells us whether
...
vs $\approx 264$). Humans don't just do better; they consider a
completely different number of possible answers. Second,
...
$\approx 281$ grids. Even when nobody gets it right, the human
candidate set stays small; the model's does not.

The Pearson correlation between per-task VARC entropy and human entropy
is $r = 0.21$ ($p = 3.1 \times 10^{-5}$, $n = 400$). This is small:
tasks that are hard for humans and tasks that are hard for VARC are
mostly different tasks. They fail on different things, not the same
...
\texttt{both\_wrong} ($U = 3577$, $p = 0.56$), showing the model's
confusion does not track whether the task is actually solvable.
...
diffuse than humans; the ten circled points have the largest $\Delta H$.}
...
plausible but conflicting transformation rules. VARC's failures are
nearly spread out over hundreds of grids per task. The model is not
picking the wrong rule from a short list; there is no short list.
...
``model is confused.'' Section~3.2 shows this signal is real but
limited: it cleanly separates VARC-correct from VARC-wrong, but cannot
separate \texttt{human\_only} from \texttt{both\_wrong}. The model is
equally confused on tasks that humans can solve and on tasks nobody can
solve. Without the human-side baseline, self-reported model confidence
does not identify the most important failures.
...
configuration; the full ensemble may behave differently. Human entropy
estimates are limited by the $\approx 10$ participants per task.

\subsection{Future work}

A natural next step is to apply targeted augmentations to the
\texttt{human\_only} tasks and measure whether any structured change
sharpens VARC's distribution. Since VARC is already maximally diffuse
on these tasks, the question is whether \emph{any} intervention helps;
if not, the failure is not about a specific invariance. Another
direction is to use the entropy gap as a signal for building harder ARC
evaluation sets.

\subsection{Conclusion}

On tasks that humans solve but VARC fails on, the two agents fail in
fundamentally different ways. Humans converge: their final answers
cluster on a handful of grids, with one clearly dominant choice. VARC
wanders: its 510 view-level predictions spread across hundreds of
distinct grids, with no candidate exceeding $13\%$ of the vote. The
difference is not about precision; it is about \emph{whether a
hypothesis was ever formed}.
