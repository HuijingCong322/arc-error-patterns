...
下图直观展示了这个分布——VARC 最常见的错误是 `wrong_position`（7/20），人类最常见的是 `partial_rule`（11/20）：

![Semantic Error Distribution](results/semantic_error_dist.png)

---

## Case Studies：3 道精选任务的深度分析

从 20 道标注任务中，我们挑选了 3 道做进一步的深度分析。选取标准：覆盖三种典型的"人类 vs. VARC 失败模式组合"，让对比尽量有代表性。

### 为什么是这三道题？

| Task | VARC 错误 | Human 错误 | 为什么选它 |
|------|-----------|------------|-----------|
| `e6de6e8f` | `near_miss` | `near_miss` | **两者以相同方式失败**：VARC 和人类都画出了几乎正确的形状，只差 1-2 个格子。说明这道题的规则本身就接近人类和 AI 的感知边界。 |
| `ad7e01d0` | `wrong_position` | `partial_rule` | **两者以不同方式失败**：VARC 找到了正确的颜色，但完全放错了位置；人类理解了规则的一部分（颜色分布），但坐标同样混乱。失败模式不同，说明两者对规则的"理解入口"不一样。 |
| `891232d6` | `wrong_rule` | `partial_rule` | **VARC 失败比人类更彻底**：人类至少知道需要连线（方向对），只是连错了具体路径；VARC 则画出完全无关的图案，对规则毫无把握。 |

这三道题合在一起，覆盖了：
- 人类和 AI **犯同样错误**（e6de6e8f）
- 人类和 AI **犯不同错误**（ad7e01d0）
- AI 比人类**错得更彻底**（891232d6）

### 三道题的对比分布图

下图展示了这 3 道 case study 任务各自的错误类型，VARC vs. Human：

![Case Study Error Distribution](results/case_study_error_dist.png)

### Case Study 1 — `e6de6e8f`：两者犯同样的错

**任务规则**：给定一行颜色序列作为输入，将其中的非背景颜色像素提取出来，重新排列成一个 2D 形状输出到黑色画布上。

**结果**：VARC 和几乎所有参与的人类都画出了形状接近正确的红色图案，但在边角细节上差了 1-2 个格子。Human accuracy = 0.10（极低，说明绝大多数人都没答对）。

**结论**：这道题展示了人类和 AI 在感知边界上的高度一致性——当规则简单但细节要求精确时，两者会以几乎相同的方式"差一点点"。

![Case Study e6de6e8f](results/report_fig_e6de6e8f.png)

---

### Case Study 2 — `ad7e01d0`：犯不同的错

**任务规则**：输入是一个带颜色方块的小网格（红、蓝、灰、黑），输出是将每个方块"展开"到一个大网格里——每种颜色的像素按照原来的空间关系散布在 15×15 的画布上。

**结果**：VARC 识别出了正确的颜色，但把它们放到了完全错误的位置（`wrong_position`）。人类也失败了，但失败方式不同：他们部分理解了"颜色分布有规律"这一点，但没能推导出精确的坐标映射（`partial_rule`）。

**结论**：这道题揭示了 VARC 和人类失败入口的差异——VARC 的视觉特征提取较准，但空间推理出错；人类对规则有模糊的感知，但执行时卡在了精确性上。

![Case Study ad7e01d0](results/report_fig_ad7e01d0.png)

---

### Case Study 3 — `891232d6`：AI 比人类错得更彻底

**任务规则**：一个复杂网格中有若干橙色线段和彩色端点（粉、绿、青、黄），规则是用彩色路径将各端点通过橙色通道连接起来，每段路径用对应端点的颜色填充。

**结果**：人类大多数理解了"需要连线"这个核心思路，只是连错了具体的端点对（`partial_rule`）。VARC 则画出了与规则完全无关的图案，完全没有捕捉到连线的概念（`wrong_rule`）。

**结论**：这道题是整个样本中 VARC 和人类差距最明显的一类——当规则需要对全局结构有抽象理解时，人类的直觉优势会显现出来，而 VARC 的视觉匹配策略完全失效。

![Case Study 891232d6](results/report_fig_891232d6.png)
